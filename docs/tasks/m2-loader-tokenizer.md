...
- [x] M2.1 Pin the first external model artifact revision.
...
- [x] M2.2 Add tokenizer JSON loading behind the tokenizer boundary.
...
- [x] M2.3 Fill the Qwen2.5 tokenizer fixture expectations.
...
- [x] M2.4 Define chat-template behavior for the target model.
  - Crates: `ocelotl-tokenizer`
...
- [x] M2.5 Inspect safetensors metadata without executing weights.
...
- [x] M2.6 Map artifact metadata into core model metadata.
...
- [x] M2.7 Add malformed artifact tests.
...
- [x] M2.8 Keep default tests offline.
...
- [x] M2.9 Document artifact preparation.
  - Crates: docs only
...
All M2.1–M2.9 tasks landed and exit criteria are met. Acceptance is provable
from `cargo test --workspace` against main through commit `0a63350 merge: M2.4
— chat-template behavior for Qwen2.5 (dev-03)` (85 default tests + 2
`#[ignore]`'d opt-in tests passing). Default tests remain offline by
construction; the `ci/check-offline.ps1` static-scan added by M2.8 now gates
PRs against accidental network-fetch additions.

Per-criterion mapping lives in `docs/validation/test-matrix.md` (M2 Acceptance
Traceability) — that table is the authoritative status surface for M2; this
