...
- [x] M5.1 Define contiguous KV cache metadata.
...
- [x] M5.2 Allocate a per-request contiguous cache.
...
- [x] M5.3 Add KV write tests for prefill.
...
- [x] M5.4 Add KV read/append tests for decode.
  - Crates: `ocelotl-models`, `ocelotl-runtime`
...
- [x] M5.5 Enforce context-length and capacity errors.
  - Crates: `ocelotl-runtime`
...
- [x] M5.6 Preserve M3 output parity.
  - Crates: `ocelotl-models`, `ocelotl-runtime`
...
- [x] M5.7 Clean up on cancellation and failure.
  - Crates: `ocelotl-runtime`
...
M5 is closed on the CPU/reference path. `ocelotl-core::KvCacheLayout` defines
the shared layout contract, `ocelotl-runtime::ContiguousKvCache` owns
request-scoped storage, and `Qwen2_5Model::{prefill_with_cache,
decode_token_with_cache}` writes and reads cache entries through an
Ocelotl-owned trait. Runtime helpers `prepare_qwen2_5_contiguous_cache` and
`decode_one_token_with_contiguous_cache` prove cached decode preserves the
M3 no-cache greedy token while appending the next cache position.
