...
| 5 | Gemma4 GGUF metadata converts into a model-family config while preserving Gemma-specific context, sliding-window, shared-KV, softcap, tokenizer, and quantization facts. | `ocelotl_models::gemma::gemma4::tests::{try_from_gguf_manifest_accepts_gemma4_fixture_and_preserves_features,try_from_gguf_manifest_rejects_missing_tokenizer_metadata,try_from_gguf_manifest_rejects_non_gemma4_architecture,try_from_gguf_manifest_rejects_invalid_sliding_window_before_compute}` validate `fixtures/metadata/gemma4_e4b_it_q4_k_m_gguf_metadata.json`. `local_gemma4_q4_k_m_gguf_header_converts_to_gemma4_config` is ignored and passed locally against `D:\Dev\ideas\04-granola-ai-clone\models\google_gemma-4-E4B-it-Q4_K_M.gguf` on 2026-05-13. | green |
| 6 | Qwen3.5 metadata is recognized separately from Qwen2.5 and preserves hybrid/MoE/multimodal/FP8 facts. | `ocelotl_models::qwen::qwen3_5::tests::{parse_qwen3_5_config_recognizes_family_separately_from_qwen2_5,parse_qwen3_5_rejects_qwen2_config_with_typed_unsupported,qwen2_5_config_rejects_qwen3_5_metadata,parse_qwen3_5_rejects_layer_type_count_mismatch_before_compute}` validate `fixtures/metadata/qwen3_5_35b_a3b_fp8_config.json` and prove the M3 Qwen2.5 gate still rejects Qwen3.5 metadata. | green |
| 7 | Unsupported Qwen3.5/Gemma4 execution features fail before compute. | `ocelotl_models::gemma::gemma4::tests::ensure_supported_for_execution_rejects_gemma4_quantized_multimodal_features` rejects Gemma4 multimodal/sliding-window/shared-KV/softcap/Q4_K_M execution. `ocelotl_models::qwen::qwen3_5::tests::ensure_supported_for_execution_rejects_qwen3_5_hybrid_moe_multimodal_fp8` rejects Qwen3.5 hybrid attention, MoE, multimodal, and FP8 execution. | green |
...
| 24 | A fresh tiny.en whisper.cpp comparison is captured after W-ASR.20-W-ASR.23, and CPU follow-up gates are measurable. | `src/main.rs::tests::bench_args_default_to_scalar_and_accept_optimized_kernel_mode` pins the benchmark hook's scalar default plus optimized-mode opt-in. `docs/benchmarks/whisper-cpp.md` records the 2026-05-12 W-ASR.24 local run: Ocelotl optimized `16,648 ms` vs whisper.cpp `564 ms`, plus same-binary scalar `14,179 ms`, and defines correctness, regression, optimized-default, and CPU-competitiveness gates. W-ASR.36 later changes the example manifest back to scalar because scalar is now the winning Whisper CPU path. | green (local benchmark captured; historical performance gates were failing) |
...
| 35 | Scalar `[out, in]` linear uses row/output tiling without breaking exact Whisper token parity. | `ocelotl_kernels::tests::scalar_linear_out_by_in_handles_row_and_output_tile_tails` pins 4-row/4-output tile edge handling, and existing scalar/optimized parity tests still cover the public backend path. Senior local release proofs on 2026-05-12 passed exact token parity for tiny.en (`1,073 ms`), base.en (`1,807 ms`), small.en (`6,345 ms`), medium.en (`20,941 ms`), and large-v2 (`40,416 ms`). W-ASR.36 later tightened the whisper.cpp denominator to greedy/no-fallback; under that fairer gate, base through large-v2 remain under `<=3x` while tiny.en is about `3.26x`. | green |
| 36 | Benchmark fixtures compare Ocelotl's greedy path against whisper.cpp greedy/no-fallback instead of default beam search. | `crates/models/tests/whisper_cpp_benchmark.rs::whisper_cpp_benchmark_manifest_names_commands_inputs_and_threads` asserts the example Ocelotl command uses `--cpu-kernel-mode scalar` and the whisper.cpp command pins `-bs 1 -bo 1 -nf` with `comparison_mode = "greedy_no_fallback"`. `docs/benchmarks/whisper-cpp.md` records the 2026-05-12 local greedy whisper.cpp totals and the revised ratios. | green |
| 37 | The Whisper benchmark hook can emit decoded transcript text without making token-only timing mandatory. | `src/main.rs::tests::bench_args_default_to_scalar_and_accept_optimized_kernel_mode` pins optional `--tokenizer-path` parsing. `src/main.rs::tests::bench_whisper_output_reports_stage_timings` pins top-level `text` plus nullable tokenizer-load/text-decode timing fields. `crates/models/tests/whisper_cpp_benchmark.rs::whisper_cpp_benchmark_manifest_names_commands_inputs_and_threads` asserts the example manifest declares `local-artifacts/whisper_tiny_en/tokenizer.json`, and `tools/whisper-cpp-bench.ps1` parses Ocelotl JSON stdout into benchmark output summaries. | green |
| 38 | M4 CubeCL RoPE rejects unsupported layouts before GPU launch. | `ocelotl_kernels::cubecl_backend::tests::{wgpu_rope_rejects_invalid_shape_before_launch,wgpu_rope_rejects_non_f32_dtype_before_launch,wgpu_rope_rejects_non_contiguous_stride_before_launch}` run under `--features cubecl-wgpu` and fail through `KernelError(backend = "cubecl")` before requesting a WGPU runtime client. | green |
| 39 | M4 Qwen prefill/decode can select a CubeCL WGPU execution backend while preserving CPU reference authority. | `ocelotl_models::qwen::qwen2_5_model::tests::cubecl_wgpu_model_advertises_gpu_execution_backend_without_launch` and `ocelotl_runtime::tests::cubecl_wgpu_runtime_builds_qwen_model_with_gpu_execution_backend_without_launch` pin the no-launch selection path. Ignored local execution proofs `crates/models/tests/qwen2_5_tiny_synthetic_prefill.rs::cubecl_wgpu_prefill_matches_cpu_reference_within_tolerance` and `crates/runtime/tests/qwen2_5_tiny_synthetic_decode.rs::cubecl_wgpu_decode_one_token_matches_cpu_reference` compare CubeCL-backed Qwen output against CPU prefill/decode. Non-RoPE kernels still use CPU fallback. | green (local WGPU proof opt-in) |
